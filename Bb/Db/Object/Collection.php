...
abstract class Blueberry_Db_Object_Collection extends Blueberry_Collection {
	...
	/**
	 * The {@link Blueberry_Db_Query} that populates this collection with objects.
	 *
	...
	protected function _populate()
    {
		if($this->_hasLinkedObject()){
			if(!class_exists($this->_elementClassName)) throw new Blueberry_Db_Object_Collection_Exception('('.get_class($this).') Undefined element class '.$this->_linkedElementClass);
			...
			while($element = $this->_query->fetch(Blueberry_Db_Query::FETCH_OBJECT)){
				$this->add($element);
			}
		}
	}
    ...
    {
		return @is_subclass_of($this->_elementClassName, 'Blueberry_Db_Object');
	}
    ...
    {
		if(!$this->_name){
			$this->_name = strtolower(preg_replace('/_Collection/i', '', get_class($this)));
		}
	}
	...
	protected function _setupElementClassName()
    {
    	if(!$this->_elementClassName){
			$this->_elementClassName = preg_replace('/_Collection/i', '', get_class($this));
		}
	}
	...
	protected function _setupQuery(Blueberry_Db_Query $query = null)
    {
		if($query){
			...
			$this->_query = new Blueberry_Db_Query("SELECT * FROM `%s`", $this->_name);
		}
	}
	...
	public function delete()
    {
		foreach($this as $e){
			if(method_exists($e, 'delete')){
                ...
                //p(sprintf('Deleting object of type %s: %s', gettype($e), print_r($e, true)));

				$e->delete();

			} else throw new Blueberry_Db_Object_Collection_Exception('Cannot delete '.get_class($e).' as element of '.get_class($this).' because '.get_class($e).'::delete() is not defined.');
		}
	}
}