...
abstract class Blueberry_Form_Object {
	...
	public function clearErrors(){
		...
		if(is_object($this->row)){
			...
			if($this->row->getInputFields()->amount() < 2) $this->row->clearErrors();
	...
	public function addError($msg){
		...
		if(!$this->row) throw new Exception('Sorry, you cannot use a validator without using a Form_Row as wrapper around your input fields.');
	 ...
	 */
	public function getCleanName(){return preg_replace('/\[.*\]$/', '', $this->name);}
	...
	public function getCamelCaseName(){
		...
		foreach(explode('_', $this->getCleanName()) as $i => $name) $names[$i] = ucfirst($name);
