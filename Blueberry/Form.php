...
class Blueberry_Form extends Blueberry_Collection {
	...
	public function loadDataFromObject($o){
		...
		if(!is_object($o)){
			throw new Blueberry_Form_Exception(
				get_class($this).'::loadDataFromObject() expects object as '.
				'input parameter, '.gettype($o).' given.'
			);
		}
		if($this->isPostBack){
			return;
	...
	public function loadDataFromArray($array){
		...
		if(!is_array($array)) throw new Blueberry_Form_Exception(get_class($this).'::loadDataFromArray() expects array as input parameter, '.gettype($array).' given.');
		...
		foreach($fields as $field){
			...
			if($field->isArrayLike()){
				...
				$field->setValue(eval('return $array'.$name.';'));
    ...
    public function setClass($c){$this->class = $c;}
	...
	public function __toString(){
		...
		if(!$this->onlyShowElements){
			...
			$s = sprintf('<form class="%s" ', $this->class);
	 ...
	 */
	public function addToFlatCollection($e)
	{
		$this->_flatCollection->add($e);
	}
	...
	public function getFieldByName($name)
	{
		foreach($this->_flatCollection as $f){
			if(method_exists($f, 'getName') && $f->getName() == $name){
				return $f;
			}
		}

		throw new Blueberry_Exception(sprintf(
			'Unable to retrieve a reference to form field "%s"', $name
		));
	}
	...
	/**
	 * @todo add arraylike fieldname support.
	 */
